...
/*   Updated: 2024/09/17 21:01:49 by maabdull         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void  save_images(t_data *data)
{
	data ->nh = 0;
	data -> nw = 0;
	data ->north.img = mlx_xpm_file_to_image(data->mlx_ptr, "textures/North.xpm",&(data->nw),&(data->nh));
	...
	data ->sh = 0;
	data -> sw = 0;
	data ->south.img = mlx_xpm_file_to_image(data->mlx_ptr, "textures/South.xpm",&(data->sw),&(data->sh));
	data->south.img_pixels_ptr = (int *)mlx_get_data_addr(data->south.img,&(data->south.bits_per_pixel),&(data->south.line_len),&(data->south.endian));
	data ->eh = 0;
	data -> ew = 0;
	data ->east.img = mlx_xpm_file_to_image(data->mlx_ptr, "textures/East.xpm",&(data->ew),&(data->eh));
	data->east.img_pixels_ptr = (int *)mlx_get_data_addr(data->east.img,&(data->east.bits_per_pixel),&(data->east.line_len),&(data->east.endian));
	data ->wh = 0;
	data -> ww = 0;
	data ->west.img = mlx_xpm_file_to_image(data->mlx_ptr, "textures/West.xpm",&(data->ww),&(data->wh));
	data->west.img_pixels_ptr = (int *)mlx_get_data_addr(data->west.img,&(data->west.bits_per_pixel),&(data->west.line_len),&(data->west.endian));
}
...
void draw_image(t_data *data,int wall_length,int rays_count, int flag, float ray_angle, float ray_length)
{
	...
	while (count <(int) wall_length && x < WIDTH && y < HEIGHT && x >= 0 && y >= 0)
	{
		...
		my_pixel_put(&(data->img),x, y , get_color(data , (int) x_text,(int)y_text,ray_angle,flag));
		y_text += step;
		y++;
		count++;
	}
